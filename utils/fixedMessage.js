...
module.exports.deleteOldMessage = (msg, title) => {
...
module.exports.updateFixedMessage = (oldExists, msg, title) => {
    if (oldExists)
        db.makeQuery(`UPDATE shopMessage SET message_id = $1, channel_id = $2 WHERE guild_id = $3 AND title = $4`, 
        [msg.id, msg.channel.id, msg.guild.id, title]);
    else
        db.makeQuery(`INSERT INTO shopMessage(message_id, channel_id, guild_id, title) VALUES ($1, $2, $3, $4)`, 
        [msg.id, msg.channel.id, msg.guild.id, title]);
}